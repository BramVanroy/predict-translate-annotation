"""Fixes atag XML files after manually changing tokenization. Particularly, this script fixes the cur and id attributes.
   After modifying the manually edited files, it will check whether the number of characters (i.e. cursor positions)
   is the same between the new file and the original file.
            ...
            for src_seg_id, direction_d in sents_d.items():
                ...
                if self.elements_equal(src_orig_tree, src_man_tree) and \
                    ...
                    print(f"WARNING: NOT identical for {identifier}, aligned pair {src_seg_id}-{tgt_seg_id}."
                          f" After uploading files to TPR-DB/YAWAT, this sentence pair will need to be re-aligned.")

            if valid_src_segs:
                self.update_atag(identifier, valid_src_segs)
